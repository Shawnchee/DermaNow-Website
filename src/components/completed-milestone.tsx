...
export default function CompletedMilestones() {
  ...
  const CONTRACT_ADDRESS = "0x3cd514BDC64330FF78Eff7c442987A8F5b7a6Aeb"; // Replace with your contract address
    ...
    try {
      ...
      for (let i = 0; i < milestoneCount; i++) {
        ...
        if (milestone[4]) {
          ...
          if (supabaseError) {
            console.error(
              "Error fetching project title from Supabase:",
              supabaseError
            );
      ...
      <Card className="w-full max-w-7xl bg-white/90 backdrop-blur-sm border">
        <CardHeader>
          ...
          <CardDescription className="text-gray-600">
            List of milestones that have been successfully completed that you've
            contributed to.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {completedMilestones.map((milestone, index) => (
                <Card
                  key={index}
                  className="border border-green-400 bg-green-50 hover:shadow-md transition-shadow"
                >
                  ...
                  <CardContent className="space-y-2 pb-2">
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-mono text-xs text-blue-800 truncate max-w-48">
