...
export default function DiscussionSection({
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      ...
      try {
        // Check cache first
        const cachedName = localStorage.getItem(`userName-${formattedAddress}`)
        if (cachedName) {
        ...
        if (data?.name) {
          ...
          localStorage.setItem(`userName-${formattedAddress}`, data.name)
        } else {
          const manualName = prompt("Please enter your display name:") || "Anonymous"
          setUserName(manualName)
          localStorage.setItem(`userName-${formattedAddress}`, manualName)
          ...
          <div className="mb-6">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                {!walletAddress && (
                  <p className="text-sm text-amber-600 mt-2">Please connect your wallet to join the discussion</p>
                )}
              </div>
            </div>
          </div>
